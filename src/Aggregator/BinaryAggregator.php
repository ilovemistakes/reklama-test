 ...
 * Сделан для уменьшения количества считываемых данных при поиске в индексе
 * и с заделом на оптимизацию.
 *
 ...
 */
class BinaryAggregator implements AggregatorInterface {
    public function run(OutputInterface $output, InputStream $in, OutputStream $out) {
        ...
        $index_filename = tempnam(sys_get_temp_dir(), 'index');
        $buffer_filename = tempnam(sys_get_temp_dir(), 'buffer');

        $index = new Index($index_filename);
